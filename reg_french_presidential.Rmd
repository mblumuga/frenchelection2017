...
#xxx <- socio %>%
# mutate_if(.,startsWith(names(.),"Nb"), funs(./socio$Population))# %>%
#  divide_by(socio$Population)
...
plot(aux$Score_LePen,pnorm(myfit$fitted.values)*100,pch=20,cex=.3);abline(0,1)

require(ranger)
myfit2 <- ranger(ans ~ ., data=aux2,importance = 'impurity')
cor(aux$Score_LePen,pnorm(myfit2$predictions)*100)^2
plot(aux$Score_LePen,pnorm(myfit2$predictions)*100,pch=20,cex=.3);abline(0,1)
varImp(myfit2)
```
```{r}
require(ggplot2)
require(ggrepel)
df <- data.frame(score = aux$Score_LePen, predicted = pnorm(myfit2$predictions) * 100, pop = aux$Population, names = ifelse(aux$Population>100000,as.character(aux$Commune),NA)) %>%
  subset(pop>5000)
  
ggplot(df, aes(x = score, y = (predicted-score)/score, label = names)) +
  geom_point(data = subset(df, pop < 100000), alpha=0.6, color ="grey") +
  geom_point(data = subset(df, pop >= 100000), alpha = 1, color ="red") +
